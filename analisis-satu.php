...
<?php
	...
	{
		echo '<table class="table  table-bordered table-highlight" style="text-align:left;">';
		for ($i=0;$i<count($arr);$i++) {
			echo '<tr>';
				echo '<td width="40">'.($i+1).'</td>';
				echo '<td style="margin:100px;">'.$arr[$i].'</td>';
			echo "</tr>";
		}
		echo '</table>';
	}
	...
	while ($dataalternatif = mysql_fetch_array($queryalternatif))
	{
		...
		while ($datakriteria = mysql_fetch_array($querykriteria))
		{
			...
			// $queryalternatifkriteria = mysql_query("
			// 		SELECT t.*, a.nama_alternatif FROM talternatif_kriteria
			// 		LEFT JOIN talternatif ON t.id_alternatif = a.id_alternatif 
			// 		WHERE t.id_alternatif = '$dataalternatif[id_alternatif]' 
			// 		AND id_kriteria = '$datakriteria[id_kriteria]'");
...
<br />
<br />
Alternatif Keterampilan Futsal Terbaik : <h4 class="heading"> <span class="label label-primary"> <?php echo $alternatifrangking[0]; ?> </span></h4> Dengan Nilai Terbesar : <h4 class="heading"> <span class="label label-success"> <?php echo $hasilrangking[0]; ?></span></h4>
<br />
<br />
